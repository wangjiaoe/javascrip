  ...
  let controller={
    ...
    processGuess:function(e){
      // target:载入和显示某个标签的 href 属性命名的、名称与这个目标吻合的框架或者窗口中的文档；这个指定名称或 id 的框架或者窗口不存在，浏览器将打开一个新的窗口，给这个窗口一个指定的标记
      let location=e.target.id;
      // 只要返回的不是null，说明获得的位置是有效的（null是一个假值）
      if (location){
        // 如果玩家猜测有效，guesses就加一，
        this.guesses++;
        // 以字符串的形式将行号和列号传递给model对象的fire，仅当击中战舰时，方法fire才返回true
  ...
  // 遍历td，为td添加单击事件链接到函数controller
  for (let i=0;i<td.length;i++){
    td[i].onlick=controller.processGuess;
  };
  ...
  // 每当玩家单机handleTable时，都调用这个函数
  // function handleTable(){
  //   let guessTd=document.getElementById('td');
  //   // 另玩家猜测的值等于guess
  //   let guess=table.value;
  //   // 将猜测的值交给processGuess
